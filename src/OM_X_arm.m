...
classdef OM_X_arm < handle
    ...
    methods
        ...
        function self = OM_X_arm(dummy)

            self.dummy = dummy
            if self.dummy
                disp('WARNING: In dummy mode! This mode is only intended to test non-USB related parts of your Robot class. Make sure to not use the dummy mode for actual testing!')
                return;
            end
        ...
        function result = bulkReadWrite(self, n, addr, msgs)
            if self.dummy
                disp('WARNING: Tried to read/write to USB in dummy mode! This mode is only intended to test non-USB related parts of your Robot class. Make sure to not use the dummy mode for actual testing!')
                result = zeros(1,4);
                return;
            end
...
%                 
                    ...
                    % ONLY IF Measuring position --Sam Honor
                    if id == self.motorIDs(2) && addr == DX_XM430_W350.CURR_POSITION
                        result(i) = result(i) - self.motor2_offset;
                    elseif id == self.motorIDs(3) && addr == DX_XM430_W350.CURR_POSITION
                        result(i) = result(i) - self.motor3_offset;
                    elseif id == self.motorIDs(3) || id == self.motorIDs(2)
                        % Do nothing
                    end
                end
                ...
                if addr == DX_XM430_W350.GOAL_POSITION
